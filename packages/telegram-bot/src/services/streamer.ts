  ...
  private buildProgressHtml(): string {
    ...
    if (this.completedTools.length > 0) {
      ...
      for (const t of recent) {
        parts.push(`<code>${t}</code>`);
      }
    }

    if (this.activeToolName && this.activeToolIntent) {
      parts.push(`⚙️ <b>Running:</b> <code>${this.activeToolIntent}</code>`);
    }
  ...
  async finalize(): Promise<void> {
    ...
    if (this.enableReactions && this.replyToMessageId) {
      ...
      if (reactions.length > 0) {
        ...
        void this.client.setMessageReaction(this.chatId, this.replyToMessageId, reactionObjs);
      }
    }
    ...
    if (this.enableStickers) {
      ...
      for (const sticker of stickers.slice(0, 2)) {
        void this.client.sendSticker(this.chatId, sticker, {
          message_thread_id: this.messageThreadId,
          reply_to_message_id: this.replyToMessageId,
        });
      }
    ...
    try {
      ...
      const plainFallback = cleanFinal.slice(0, 3800) || "(No output)";
      if (this.sentMessage) {
        void this.client.editMessageText(this.chatId, this.sentMessage.message_id, plainFallback);
      } else {
        void this.client.sendMessage(this.chatId, plainFallback, {
          message_thread_id: this.messageThreadId,
          reply_to_message_id: this.replyToMessageId,
        });
      }
